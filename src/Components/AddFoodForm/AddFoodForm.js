...
class AddFoodForm extends Component {
    ...
    if (this.props.id) {
      ...
      axios
        ...
        .then(res => {
          ...
          axios
            .get(`/api/food/${currentMeal.meal_id}`)
            .then(res => {
              this.setState({
                foodList: res.data,
  ...
  search = async val => {
    let res = await axios
      .get(
        `https://api.nal.usda.gov/ndb/search/?format=json&q=${val}&max=25&sort=n&api_key=ypLihhr1bxOT6RSZHDLPLbxzMVleTd0VaWKV9a8h`
      )
      .catch(err => console.log("error in add food form", err));
    const initialList = res.data.list.item;
    let resultsList = [];
    initialList.map(item => {
      let name = item.name.slice(0, -19).replace(/\w\S*/g, function(txt) {
  ...
  addToState = async ndbno => {
    const res = await axios
      .get(
        `https://api.nal.usda.gov/ndb/V2/reports?ndbno=${ndbno}&type=b&format=json&api_key=ypLihhr1bxOT6RSZHDLPLbxzMVleTd0VaWKV9a8h`
      )
      .catch(err => console.log("error in add food form", err));
    ...
    let calories = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "208"
    );
    let protein = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "203"
    );
    let carbs = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "205"
    );
    let fat = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "204"
    );
    let sugar = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "269"
    );
    let fiber = result.food.nutrients.find(
      nutrient => nutrient.nutrient_id === "291"
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Body>
          <MealTitle>
            {this.props.date} Meal {this.props.number}
          </MealTitle>
          ...
          <Search>
            ...
            <ButtonLink
              onClick={() => this.props.clearCurrentMeal}
              to="/meallog"
            >
              done
            </ButtonLink>
          ...
          <div>
            {this.state.resultsList.length !== 0 &&
              this.state.resultsList.map((food, i) => {
                let { name } = food;
                name = name.slice(0, -19).replace(/\w\S*/g, function(txt) {
                  return (
                ...
                return (
                  <ResultList
                    key={i}
                    onClick={() => this.handleAddFood(food.ndbno)}
                  >
                    <p>{name}</p>
                    <button>></button>
                  </ResultList>
...
export default connect(
  mapStateToProps,
  { setCurrentFood, setFoodSearch, clearCurrentMeal }
)(AddFoodForm);
